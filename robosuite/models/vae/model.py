...
class ConvVAE(object):
    ...
    def _build_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.input_tensor = tf.placeholder(tf.float32, shape=[None, IMAGE_WIDTH, IMAGE_HEIGHT, 3])
            print("self.input_tensor shape: ", self.input_tensor.shape)

            # Encoder
            h1 = tf.layers.conv2d(self.input_tensor, 32, 4, strides=2, activation=tf.nn.relu, name="enc_conv1", padding='same')
            print("encoder h1 shape: ", h1.shape)
            h2 = tf.layers.conv2d(h1, 64, 4, strides=2, activation=tf.nn.relu, name="enc_conv2", padding='same')
            print("encoder h2 shape: ", h2.shape)
            h3 = tf.layers.conv2d(h2, 128, 4, strides=2, activation=tf.nn.relu, name="enc_conv3", padding='same')
            print("encoder h3 shape: ", h3.shape)
            h4 = tf.layers.conv2d(h3, 256, 4, strides=2, activation=tf.nn.relu, name="enc_conv4", padding='same')
            print("encoder h4 shape: ", h4.shape)
            # h = tf.reshape(h, [-1, 3 * 8 * 256])
            h5 = conv_to_fc(h4)
            print("encoder h5 shape: ", h5.shape)

            # VAE
            self.mu = tf.layers.dense(h5, self.z_size, name="enc_fc_mu")
            self.logvar = tf.layers.dense(h5, self.z_size, name="enc_fc_log_var")
            ...
            # Decoder
            h5 = tf.layers.dense(self.z, h4.shape[1] * h4.shape[2] * 256, name="dec_fc")
            print("decoder h5 shape: ", h5.shape)
            h6 = tf.reshape(h5, [-1, h4.shape[1], h4.shape[2], 256])
            print("decoder h6 shape: ", h6.shape)
            h7 = tf.layers.conv2d_transpose(h6, 128, kernel_size=4, strides=2, activation=tf.nn.relu, name="dec_deconv1", padding='same')
            print("decoder h7 shape: ", h7.shape)
            h8 = tf.layers.conv2d_transpose(h7, 64, kernel_size=4, strides=2, activation=tf.nn.relu, name="dec_deconv2", padding='same')
            print("decoder h8 shape: ", h8.shape)
            h9 = tf.layers.conv2d_transpose(h8, 32, kernel_size=4, strides=2, activation=tf.nn.relu, name="dec_deconv3", padding='same')
            print("decoder h9 shape: ", h9.shape)
            self.output_tensor = tf.layers.conv2d_transpose(h9, 3, kernel_size=4, strides=2, activation=tf.nn.sigmoid,
                                                            name="dec_deconv4", padding='same')
            print("decoder self.output_tensor shape: ", self.output_tensor.shape)                                                            
            ...
            if self.is_training:
                ...
                # reconstruction loss
                print("self.input_tensor shape: ", self.input_tensor.shape)
                print("self.output_tensor shape: ", self.output_tensor.shape)
